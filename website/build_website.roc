...
ensure_examples_present! : () => Try({}, _)
ensure_examples_present! = || {
	if path("content/examples").is_dir!()? {
		Ok({})
	} else {
		run!("curl", ["-fL", "-o", "examples-main.zip", "https://github.com/roc-lang/examples/archive/refs/heads/main.zip"])?
		run!("unzip", ["-o", "-q", "examples-main.zip"])?
		run!("cp", ["-R", "examples-main/examples/", "content/examples/"])?
		replace_all_in_file!("content/examples/index.md", "](/", "](/examples/")?
		path("examples-main").delete_all!() ?? {}
		path("examples-main.zip").delete!() ?? {}
		Ok({})
	}
}
...
add_github_links_to_examples! = || {
	if !path("build/examples").is_dir!()? {
		...
		patch_example_readmes!(readme_paths)
	}
}

patch_example_readmes! : List(Str) => Try({}, _)
patch_example_readmes! = |paths|
	match paths {
		[] => Ok({})
		[first, .. as rest] => {
			...
			link = "https://github.com/roc-lang/examples/tree/main/examples/${example_name}"
