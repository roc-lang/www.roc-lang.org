#!/usr/bin/env python3
"""Update the pinned Roc nightly release used by the website and installer.
...
version date, build id, base URL and SHA256 checksums in:
  - .roc-version
  - website/public/install_roc.py

The script edits only the specific variable assignments, so unrelated changes to
the installer are preserved. It exits 0 whether or not anything changed; the CI
...
INSTALLER_PATH = os.path.join(REPO_ROOT, "website", "public", "install_roc.py")
...
# instead of failing the update, and the installer tells users that these targets
...
def update_installer(info):
    ...
    text = replace_assignment(
        text,
        r'VERSION_DATE = "[^"]*"',
        f'VERSION_DATE = "{info["version_date"]}"',
        INSTALLER_PATH,
    )
    text = replace_assignment(
        text,
        r'BUILD_ID = "[^"]*"',
        f'BUILD_ID = "{info["build_id"]}"',
        INSTALLER_PATH,
    )
    text = replace_assignment(
        text,
    ...
    text = replace_assignment(
        text,
        r'SHA_LINUX_ARM64 = "[^"]*"',
        f'SHA_LINUX_ARM64 = "{info["shas"]["linux_arm64"]}"',
        INSTALLER_PATH,
    )
    text = replace_assignment(
        text,
        r'SHA_LINUX_X86_64 = "[^"]*"',
        f'SHA_LINUX_X86_64 = "{info["shas"]["linux_x86_64"]}"',
        INSTALLER_PATH,
    )
    text = replace_assignment(
        text,
        r'SHA_MACOS_ARM64 = "[^"]*"',
        f'SHA_MACOS_ARM64 = "{info["shas"]["macos_apple_silicon"]}"',
        INSTALLER_PATH,
    )
    text = replace_assignment(
        text,
        r'SHA_MACOS_X86_64 = "[^"]*"',
        f'SHA_MACOS_X86_64 = "{info["shas"]["macos_x86_64"]}"',
        INSTALLER_PATH,
    )
    text = replace_assignment(
        text,
        r'SHA_WINDOWS_X86_64 = "[^"]*"',
        f'SHA_WINDOWS_X86_64 = "{info["shas"]["windows_x86_64"]}"',
        INSTALLER_PATH,
    )
    ...
    if "windows_arm64" in info["shas"]:
        text = replace_assignment(
            text,
            r'SHA_WINDOWS_ARM64 = "[^"]*"',
            f'SHA_WINDOWS_ARM64 = "{info["shas"]["windows_arm64"]}"',
            INSTALLER_PATH,
        )
...
def main():
    ...
    update_installer(info)
    print("Updated .roc-version and install_roc.py")
