...
async function checkPage(page, url) {
  ...
  page.on('console', msg => {
    ...
    // Ignore known 401 on the tutorial page (unrelated third-party resource)
    if (url === `${WEBSITE_URL}/tutorial` && text.includes('the server responded with a status of 401')) {
      return;
    }
